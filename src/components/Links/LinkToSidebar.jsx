...
function LinkToSidebar({ link, icon, title, current }) {
    return (
        <li aria-current={current ? 'page' : undefined}>
            <Link to={link}
                className={`${current && "bg-white text-blue-500 border-blue-500"} relative flex flex-row items-center h-11 focus:outline-none hover:bg-white " +
